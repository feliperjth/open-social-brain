...
function setView(view) {
  ...
  const views = {
    ...
    "medial-left": cameraPose(new THREE.Vector3(0.92, 0.05, 0.38), 1.44),
    "medial-right": cameraPose(new THREE.Vector3(-0.92, 0.05, 0.38), 1.44),
...
function setMedialCut(hemisphere) {
  ...
  allAtlasMeshes.forEach((mesh) => {
    ...
    // In medial view, BackSide inner meshes z-fight and overwrite surface colour during rotation.
    // Hide them while the clip plane is active; restore via syncInternalSurface on exit.
    const inner = mesh.userData.innerMesh;
    if (inner) {
      if (medialCutEnabled) {
        inner.visible = false;
